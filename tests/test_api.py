...
def test_the_wake_count_is_read_out_of_the_workflow_files(client):
    ...
    Counting only the daily ones reported the watch as free.

    The probe is zero because it ships with no `schedule:` at all - only the
    commented-out one in its header, which must not be counted. Update these
    numbers if you change a workflow's crons."""
    app, _ = client
    wakes = app.get("/api/schedules").json()["wakes_per_day"]
    assert wakes["scrape.yml"] == 3
    assert wakes["watch.yml"] == 6
    assert wakes["probe.yml"] == 0
...
def test_an_idle_repo_is_costed_before_a_single_search(client):
    ...
    # Three sweep slots and six watch slots; the probe adds nothing until it is
    # given a schedule of its own.
    assert body["idle_minutes_if_all_active"] == 9 * 30
