...
def test_a_budget_ends_the_sweep_and_keeps_what_it_found(tmp_path):
    ...
    # `run_sweep_command` loads a real file from `scenarios/`, and the example
    # one is edited through the UI like any other - so the plan size is not
    # something this test can pin down. The margin is made per-search instead:
    # one search alone overruns the 60 ms budget, which holds at any plan size.
    # Retarget this at one of your own trips if you delete the example.
    result = run_sweep_command(
        "example-japan-philippines", None, dry_run=False, mode="explore",
