...
from src.probe import PROBE_ROUTES, probe_report, record_observation, run_probe
...
def test_the_probe_ships_with_no_routes():
    """Routes are fixed rather than read from a trip, so that observations stay
    comparable over the whole collection period. That is also why they cannot be
    inherited: someone else's routes measure someone else's trip. Fill
    `PROBE_ROUTES` in before switching the workflow on."""
    assert PROBE_ROUTES == []


def test_a_probe_with_no_routes_says_so_instead_of_searching(tmp_path, capsys):
    """Not a silent empty run. It stops before opening a browser, and writes no
    observation file: a run that does nothing and still looks successful is how
    a dead workflow goes unnoticed."""
    assert run_probe(tmp_path) == []
    assert "No routes are set" in capsys.readouterr().out
