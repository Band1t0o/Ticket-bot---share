...
> **New here? Open [MANUAL.html](MANUAL.html) first.** It is the step-by-step guide: set the thing
> up, cast a wide net, cut it down, and watch the price of the trip you settled on. This README is
> the reference behind it — why each decision was made, and which bug forced it. The manual tells
> you what to press; this tells you why it behaves as it does.
...
**How easy an airport is to get to is a third axis, and nothing knew it.** One airport is a tram
ride, the next is a morning, the third is a coach — and frequency cannot discover that, because the
usual reason a convenient airport goes unused is that it has no long-haul inventory. The regional
airport twenty minutes away is often exactly that: convenient, and flagged *no inventory* by
`viability.py` in the same chip.
...
nothing sweeps an airport it cannot use — live in `data/airport_notes.json`. **It does not ship**,
because these are measurements of one person's local airports and they would be wrong for yours.
Write your own as you learn them:

```json
{
  "_measured": "2027-01 against pelikan.cz, one adult, on a sample date",
  "airports": {
    "VIE": { "verdict": "ok",           "note": "cheapest of the three measured" },
    "BTS": { "verdict": "no_return",    "note": "nothing sold coming back" },
    "BRQ": { "verdict": "no_inventory", "note": "no long-haul either direction" }
  }
}
```

`ok`, `no_return` and `no_inventory` are the three verdicts, and the note is shown verbatim on the
airport's chip. A missing file simply means no hand-written notes, and everything derived from your
own sweeps carries on regardless.

The finding worth writing down is the one that stops you re-litigating an airport in six months:
that the convenient regional airport sells no long-haul at all is not something a sweep will ever
tell you twice, because after the first time you stop sweeping it.
...
**Keep the repo public.** Actions minutes are free on a public repo and capped at 2,000 a month on
a private one, and the schedule below would eat most of that allowance without searching anything.
That single fact is what lets depth go unrationed. If the repo has to be private, pause the
schedule first — see *Stopping all of it* below.
...
```bash
gh secret set DISCORD_WEBHOOK_URL -R <your-github-user>/<your-repo>
```
...
Started by **Vaclav Korvas** in October 2025 as a Discord bot, and rewritten from there into the
multi-leg sweeper it is now. This copy carries no history, so the record lives here instead.
