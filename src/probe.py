...
The sweep cadence should be set from data rather than intuition. A round trip
read twice about twenty minutes apart came back 3.3% dearer the second time,
which shows intraday movement exists but says nothing about how often or how far
prices really move. This samples a few fixed routes every couple of hours so the
question can be answered. It is deliberately tiny (~2 min per run).

It earned its place twice in the repo it came from: it established that fares
move day-over-day rather than intraday, which is what set the sweep cadence, and
it caught one route climbing 25% in four days.

**It ships switched off, with no routes**, and both reasons have to be answered
before turning it on. Routes are fixed rather than derived from a trip, so that
observations stay comparable across the whole collection period - which also
means someone else's routes measure someone else's trip and tell you nothing
about yours. And every two hours is ~720 Actions minutes a month: free on a
public repo, and more than a third of the allowance on a private one.

To use it, put your own routes in `PROBE_ROUTES` below, pick dates far enough
out that they will not pass while you are still collecting, and add a `schedule:`
back to `.github/workflows/probe.yml`, which is the authority on whether it runs.
Once it has a schedule, *Pause all scheduled runs* in the gear stops it with
everything else.
"""
...
# Your own routes, as (origin, destination, departure date). Three is what the
# original set used, and the shape is worth copying: two ways of reaching the
# same place, plus one leg further along the trip, so that a fare moving can be
# told apart from one carrier moving.
#
#   PROBE_ROUTES = [
#       ("VIE", "HND", date(2027, 1, 12)),
#       ("PRG", "HND", date(2027, 1, 12)),
#       ("KIX", "CEB", date(2027, 1, 26)),
#   ]
...
def run_probe(directory: Path | str = DEFAULT_PROBE_DIR, provider=None) -> list[dict]:
    ...
    Says so and stops when there are none, rather than opening a browser to
    search nothing and then writing an observation file with no observations in
    it. A run that does nothing and reports success is the shape that let a dead
    workflow go unnoticed for eight months in the repo this came from.
    """
    if not PROBE_ROUTES:
        print(
            "[probe] No routes are set, so there is nothing to sample. Put your own "
            "in PROBE_ROUTES in src/probe.py - the note at the top of that file says how."
        )
        return []
    if provider is None:
